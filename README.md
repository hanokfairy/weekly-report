...
> 이미 설치가 끝나서 **관리자 페이지 사용법만** 필요하다면 [USER_GUIDE.md](USER_GUIDE.md)를 보세요. 이 README는 최초 설치·설정(API 키 발급, GitHub Actions, 서버 실행 등)을 다룹니다.
...
가장 쉬운 방법은 **8단계의 관리자 페이지**에서 폼으로 추가하는 것입니다. 코드 편집 없이도 가능합니다. 직접 JSON 파일을 편집하고 싶다면 아래 방법을 따르세요.
...
```json
{
  ...
  "blogs": [
    {
      "name": "메인 블로그",
      "rss_url": "https://rss.blog.naver.com/블로그아이디.xml",
      "blog_id": "블로그아이디",
      "keywords": ["이 블로그로 체크할 키워드1", "키워드2"],
      "max_rank": 30
    },
    {
      "name": "서브 블로그 (있는 경우만)",
      "rss_url": "https://rss.blog.naver.com/서브블로그아이디.xml",
      "blog_id": "서브블로그아이디",
      "keywords": ["서브 블로그 전용 키워드1"],
      "max_rank": 30
    }
  ],
...
- **블로그는 `blogs` 배열에 원하는 만큼 추가할 수 있고, 블로그마다 자기 것만의 `keywords`를 가집니다.** 예를 들어 메인 블로그는 "A키워드"로 순위를 체크하고 서브 블로그는 "B키워드"로 체크하고 싶다면, 각 블로그 항목의 `keywords`를 다르게 넣으면 됩니다. 키워드 순위는 그 블로그의 `blog_id`가 검색 결과에 노출되는지로 체크합니다.
- `blog_id`: 네이버 블로그 주소 `blog.naver.com/이부분`
- `alert_threshold_pct`: 전주 대비 유입이 이 %만큼 변하면 관련 뉴스를 자동 검색 (기본 15)
- `news_keywords`: 유입 급변 시 검색할 진료과/이슈 키워드 (비워두면 각 블로그 `keywords`를 모두 합친 것 중 앞 3개를 자동 사용)
- `slack.channel_id`: 3단계 8~9번에서 확인한 채널 ID (실제로 전송에 사용되는 값). `channel_name`은 사람이 보기 위한 메모일 뿐 프로그램에서 사용하지 않습니다.

각 병원마다 Slack으로 `[병원명_날짜 주간보고]` 라는 제목의 `.txt` 파일이 개별 전송되고, 보고서 안의 "주요 관리 키워드 노출 현황"은 블로그별로 나뉘어 표시됩니다.
...
### 7-3. ezVPN 연결이 필요한 경우

플레이스 로그인에 ezVPN이 필요하다면, **스크립트를 실행하기 전에 먼저 ezVPN 앱을 열어 계정으로 로그인/연결**해두세요. ezVPN은 별도의 데스크톱 앱이라 이 프로그램이 대신 로그인해줄 수는 없고, Mac 전체의 인터넷 연결을 ezVPN 경유로 바꿔주는 방식이라 **사람이 먼저 연결만 해두면 스크립트는 평소처럼 동작**합니다.

혹시 깜빡하고 VPN 연결 없이 실행했을 때 이를 감지해 미리 막아주는 안전장치가 있습니다: `.env`에 아래처럼 예상 VPN IP의 앞자리를 적어두면, 실행 시점의 공인 IP가 그 대역이 아닐 경우 로그인을 시도하지 않고 바로 중단하며 Slack으로 "ezVPN 연결 확인 필요" 알림을 보냅니다 (네이버 계정에 잘못된 환경에서 로그인 시도가 쌓이는 것을 방지).

```
EXPECTED_VPN_IP_PREFIX=123.45.
```

- 확인 방법: ezVPN 연결한 상태에서 브라우저로 https://api.ipify.org 접속 → 나온 IP의 앞자리(예: `123.45.`)를 적으면 됩니다.
- 비워두면 이 확인 절차 없이 그냥 진행합니다 (VPN이 필요 없는 병원이라면 비워두세요).
- **한계**: launchd로 새벽/이른 아침에 자동 실행되도록 예약해도, ezVPN 로그인 자체는 사람이 미리 해둬야 합니다. 완전 무인 자동화를 원하시면 ezVPN이 계정 자동 로그인이나 상시 연결 기능을 지원하는지 IT 담당자에게 확인해보시는 걸 추천드립니다.
...
| `scrape_smart_place.py` 실행 시 "ezVPN 연결 확인이 필요합니다" 중단 메시지 | ezVPN 앱을 열어 로그인/연결 후 다시 실행. `.env`의 `EXPECTED_VPN_IP_PREFIX` 값이 실제 VPN IP 대역과 맞는지도 확인 |
